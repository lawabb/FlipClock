...
void Clock::DrawTime()
{
  ...
  for (int digit = 0; digit <= 3; digit++)
    for (int step = 0; step <= digit; step++)
      BigDigit(digitOriginX + digit*interDigitX, BigDigitY(), 1, step);
  delay(10000L);
#else
...
void Clock::BigDigit(int x, int y, int digit1, int step)
{
  // draw digit1 at x,y at the given step of flipping to the next digit
  int digit2 = digit1 + 1;
  if (digit2 > 9)
    digit2 = 0;
  if (step == 0)  // normal
...
void Clock::updateTime()
{
  ...
  for (int digit = 3; digit >=0; digit--)
  {
    if (newDigits[digit] != currentDigits[digit])
    {
      ...
      {
        unsigned long start = millis();
        BigDigit(digitOriginX + digit*interDigitX, BigDigitY(), currentDigits[digit], step);
