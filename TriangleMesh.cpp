...
#define fastTFT_Window(_X, _Y, _X2) ILI948x::Window(_X, _Y, _X2, screenHeight);
...
void TriangleMesh::Paint()
{
  ...
  if (flags & newMeshFlag)
  {
    ...
    // blank upper
    fastTFT_Window(0, 0, screenWidth);
    ...
    // blank lower
    fastTFT_Window(0, screenHeight - edgeHeight - maxVertexOffset, screenWidth);
    fastTFT_BlackPixels(screenWidth*(edgeHeight + maxVertexOffset));
